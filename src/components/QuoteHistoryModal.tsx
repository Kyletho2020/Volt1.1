...
import { Archive, Search, Trash2, Download, Save, X, Calendar, Building, User, FileText, Loader, CheckCircle, AlertCircle, Plus, Mail } from 'lucide-react'
...
const QuoteHistoryModal: React.FC<QuoteHistoryModalProps> = ({
  isOpen,
  onClose,
  onLoadQuote,
  ...
  const generateNewQuoteNumber = () => {
    const newNumber = QuoteService.generateQuoteNumber(
      currentEquipmentData.projectName,
      currentEquipmentData.companyName
    )
  ...
  const handleOverwriteQuote = async () => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border-2 border-accent rounded-2xl shadow-2xl w-full max-w-6xl h-[85vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b-2 border-accent">
          <div className="flex items-center">
            <Archive className="w-6 h-6 text-white mr-2" />
            <h3 className="text-xl font-bold text-white">Quote History</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* Save Section */}
          <div className="w-1/3 p-6 border-r-2 border-accent">
            <h4 className="text-lg font-semibold text-white mb-4">Save Current Quote</h4>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-black rounded-lg p-4 border border-accent">
                ...
                <div className="space-y-1 text-sm text-gray-300">
                  <div className="flex items-center">
                    ...
                    <span>{currentEquipmentData.projectName || 'No project name'}</span>
                  </div>
                  <div className="flex items-center">
                    <Building className="w-4 h-4 mr-2" />
                    <span>{currentEquipmentData.companyName || 'No company'}</span>
                  </div>
                  <div className="flex items-center">
                    <User className="w-4 h-4 mr-2" />
                    <span>{currentEquipmentData.contactName || 'No contact'}</span>
                  </div>
                  <div className="flex items-center">
                    <Mail className="w-4 h-4 mr-2" />
                    <span>{currentEquipmentData.email || 'No email provided'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                {selectedQuote && (
                  <button
                    onClick={handleOverwriteQuote}
                    disabled={loading}
                    className="w-full flex items-center justify-center px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                  >
                    {loading ? (
                      <Loader className="w-4 h-4 animate-spin mr-2" />
                    ) : (
          ...
          <div className="flex-1 flex flex-col">
            <div className="p-6 border-b-2 border-accent">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-lg font-semibold text-white">Saved Quotes</h4>
              </div>
        ...
        <div className="p-6 border-t-2 border-accent bg-gray-900">
          <p className="text-sm text-gray-400">
            Click on a quote to select it, then use Load to fill forms or Overwrite to update with current data. 
            Quote numbers are auto-generated but can be customized before saving.
          </p>
        </div>
      </div>
    </div>
